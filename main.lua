...
local function draw_hud()
 ...
 print("FPS: "..stat(7),80,100,7) print("CPU: "..stat(1),80,108,7)
end
...
function _update60()
 plan_budget=1
 if game_state~=0 then if btnp(4) or btnp(5) then build_level() game_state=0 end return end

 player.hurt_cd=max(0,player.hurt_cd-1) player.fire_cd=max(0,player.fire_cd-1)
 ...
 if btn(3) then dx-=cx*move_spd dy-=cy*move_spd end
 if dx~=0 or dy~=0 then player.x,player.y=move_circle(player.x,player.y,dx,dy,0.18) end

 if key_ent and not key_ent.got and dist(player.x,player.y,key_ent.x,key_ent.y)<0.45 then key_ent.got=true player.has_key=true end
